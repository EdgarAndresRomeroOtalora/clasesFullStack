module.exports = function(db){
    ...
    router.post('/', function(request,response){
        db.serialize(function () {
              ...
              +request.body.cc+', '+request.body.email);      
        });
    });
    ...
    router.put('/', function(request,response){
        db.serialize(function () {
            db.run("UPDATE users SET name='"+request.body.nombre+"', email='"
            +request.body.email+"'WHERE id_number="+request.body.cc);  

            response.send('Se actualizo la tabla: '+TABLE+' con la información '+request.body.nombre+', '
              +request.body.cc+', '+request.body.email);      
        });
    });
